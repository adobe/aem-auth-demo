...
`ui.config/src/main/content/jcr_root/apps/aem-auth-demo/osgiconfig/config.publish/org.apache.sling.jcr.repoinit.RepositoryInitializer~saml-access.cfg.json`

#### Protected Pages and Required Groups

| Page | Path | Required External Group | Notes |
|------|------|------------------------|-------|
| **SAML Authenticated** | `/content/aem-auth-demo/us/en/saml-authenticated` | `offline_access;saml-idp` | Accessible to users with `offline_access` role via SAML |
| **OAuth2 Authenticated** | `/content/aem-auth-demo/us/en/oauth2-authenticated` | `test-group` | Accessible to users in `test-group` via OIDC |
