...
When a 500 Internal Server Error occurs in the `/content/aem-auth-demo/us/en/oauth2-authenticated` context, the Dispatcher will serve a custom error page instead of the default AEM error response.
...
2. **Custom VHost Configuration** (`dispatcher/src/conf.d/available_vhosts/aem-auth-demo.vhost`)
   - Extends the default publish virtualhost
   - Sets `DispatcherPassError 1` globally to allow Apache to handle errors
   - Includes `LocationMatch` directive for `/content/aem-auth-demo/us/en/oauth2-authenticated.*`
...
1. A request is made to `/content/aem-auth-demo/us/en/oauth2-authenticated/*`
2. If AEM returns a 500 error:
   - Because `DispatcherPassError 1` is set globally, Dispatcher passes the error to Apache
   - The `LocationMatch` directive detects the path matches `^/content/aem-auth-demo/us/en/oauth2-authenticated.*`
...
2. Verify the Dispatcher configuration:
   ...
   # Should show: aem-auth-demo.vhost -> ../available_vhosts/aem-auth-demo.vhost
   ```
...
2. Access: `http://localhost:8085/content/aem-auth-demo/us/en/oauth2-authenticated/j_security_check` with invalid parameters
...
3. Add corresponding `ErrorDocument` directives in the vhost:
   ```apache
   <LocationMatch "^/content/aem-auth-demo/us/en/oauth2-authenticated.*">
...
1. Add new `LocationMatch` directives in `aem-auth-demo.vhost`:
   ```apache
   <LocationMatch "^/content/aem-auth-demo/us/en/saml-authenticated.*">
       ErrorDocument 500 /error-500.html
   </LocationMatch>
   ```
